...
async fn send_message(client: Client, command_line_arguments: &ArgMatches) {
    ...
    let send_message_response = match client.send_messages(message_request).await {
    ...
    if let Some(error) = send_message_response.error {
        eprintln!("ERROR: Request could not be processed: {} ({})", error.description, error.code);
        process::exit(exitcode::UNAVAILABLE);
    }

    for message in send_message_response.messages() {
        match message.error {
            Some(error) =>  eprintln!( "ERROR: Message could not be processed: {} : {} ({})", message.to, error.description, error.code),
...
async fn message_status(client: Client, command_line_arguments: &ArgMatches) {
...
async fn balance(client: Client, _command_line_arguments: &ArgMatches) {
...
async fn main() {
    ...
    let client = Client::new(&api_key).expect("Could not build client");

    match command {
        SEND_MESSAGE => send_message(client, command_arguments).await,
        MESSAGE_STATUS => message_status(client, command_arguments).await,
        BALANCE => balance(client, command_arguments).await,
        unknown_command => {
